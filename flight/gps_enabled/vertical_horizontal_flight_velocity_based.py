...
from image_capture_thread import start_image_capture, stop_image_capture, gps_data
...
def monitor_altitude(master, target_alt=3.0):
    ...
    while time.time() < timeout:
        ...
        if msg:
            ...
            # Update shared gps_data for the thread
            gps_data["lat"] = lat
            gps_data["lon"] = lon
            gps_data["alt"] = alt
...
def move_forward_local(master, distance_meters):
    print("[GPS] Retrieving current GPS position...")
    msg = master.recv_match(type="GLOBAL_POSITION_INT", blocking=True)
    if msg:
        lat = msg.lat / 1e7
...
def takeoff(master, alt=3):
    ...
    start_image_capture()  # ? Start image thread here
    master.mav.command_long_send(
        master.target_system, master.target_component,
...
def land(master):
    ...
    stop_image_capture()  # ? Stop image thread on landing
    master.mav.command_long_send(
        master.target_system,
        master.target_component,
...
def main():
    ...
    move_forward_local(master, distance_meters = -2)
    land(master)
    disarm(master)
